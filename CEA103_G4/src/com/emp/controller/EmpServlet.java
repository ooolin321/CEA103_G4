...
import pk1.model.EmpService;

import pk1.model.EmpDAO;
import pk1.model.EmpVO;
		...
		if ("getOne_For_Display".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/select.jsp");
...
//		==================================================================================================
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/listAllEmp.jsp");
				failureView.forward(req, res);
			}
		}
...
//			Integer deptno = new Integer(req.getParameter("deptno").trim());
				...
				/*************************** 2.開始修改資料 *****************************************/
				EmpService empSvc = new EmpService();
				empVO = empSvc.updateEmp(empno, job, ename, id, gender, dob, addr, sal, state, hiredate);
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				try {
					...
					errorMsgs.put("dob", "請輸入日期");
				}

				String addr = req.getParameter("addr").trim();
				if (addr == null || id.trim().length() == 0) {
					errorMsgs.put("addr", "地址請勿空白");
				}
...
//					Integer deptno = new Integer(req.getParameter("deptno").trim());
				...
				/*************************** 2.開始新增資料 ***************************************/
				EmpService empSvc = new EmpService();
				empSvc.addEmp(job, ename, id, gender, dob, addr, sal, state, hiredate);
